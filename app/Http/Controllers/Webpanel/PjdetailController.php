...
class PjdetailController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($file) {
            ...
            $newLg = 'upload/pjdetail/' . $filename . '-.' . $ext;
        ...
        if ($data->save()) {
            ...
            if ($request->gallery) {
                ...
                for ($i = 0; $i < count($gallery); $i++) {
                    ...
                    $newLg = 'upload/pjdetail/gallery/' . $gfilename . '-' . $i . '.' . $ext;

                    Storage::disk('public')->put($newLg, $lg);

                    GalleryModel::insert(['_id' => $data->id, 'type' => 'pjdetail', 'image' => $newLg, 'created' => date('Y-m-d H:i:s')]);
                }
            }
    ...
    public function edit($id)
    {
        ...
        return view("$this->prefix.pages.$this->folder.index", [
            ...
            'row' => $row,
            'gallerys' => GalleryModel::where(['type' => 'pjdetail', '_id' => $id])->get(),
            'size' => $this->ImageSize(),
        ]);
    }
