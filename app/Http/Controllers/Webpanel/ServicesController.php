...
class ServicesController extends Controller
{
    ...
    {
        $data = ServicesModel::orderBy('sort');
        ...
        return view("$this->prefix.pages.$this->folder.index", [
            'js' => [
                ...
                ["type" => "text/javascript", "src" => "back-end/build/services.js"],
            ],
            'prefix' => $this->prefix,
            'folder' => 'services',
            'page' => 'index',
            'segment' => "$this->segment/services",
            'rows' => $rows
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($file) {
            ...
            $newLg = 'upload/services/' . $filename . '-.' . $ext;
            Storage::disk('public')->put($newLg, $lg);
            
            $data->image = $newLg;
        }
        if ($data->save()) {
            ServicesModel::where('id', '!=', $data->id)->increment('sort');
            // gallery
            if ($request->gallery) {
                ...
                for ($i = 0; $i < count($gallery); $i++) {
                    ...
                    $newLg = 'upload/services/gallery/' . $gfilename . '-' . $i . '.' . $ext;

                    Storage::disk('public')->put($newLg, $lg);

                    GalleryModel::insert(['_id' => $data->id, 'type' => 'services', 'image' => $newLg, 'created' => date('Y-m-d H:i:s')]);
                }
            }
            return view("$this->prefix/alert/sweet/success", ['url' => url("$this->segment/services")]);
        } else {
            return view("$this->prefix/alert/sweet/error", ['url' => url("$this->segment/services/create")]);
        }
    }
    public function edit($id)
    {
        $row = ServicesModel::find($id);
        return view("$this->prefix.pages.$this->folder.index", [
            'js' => [
                ['type' => "text/javascript", 'src' => "back-end/js/jquery.min.js", 'class' => "view-script"],
                ['src' => "back-end/tinymce/tinymce.min.js"],
            ...
            'row' => $row,
            'gallerys' => GalleryModel::where(['type' => 'services', '_id' => $id])->get(),
            'size' => $this->ImageSize(),
        ]);
    }
    ...
    {
        $datas = ServicesModel::find(explode(',', $request->id));
        if (@$datas) {
            foreach ($datas as $data) {

                ServicesModel::where('sort', '>', $data->sort)->decrement('sort');
                //destroy
                $query = ServicesModel::destroy($data->id);
            }
        }
        if (@$query) {
    ...
    public function dragsort(Request $request)
    {
        ...
        $get = ServicesModel::find($request->id);
        if ($from != "" && $to != "") {
            if ($from > $to) {
                ServicesModel::whereBetween('sort', [$to, $from])->whereNotIn("id", [$get->id])->increment('sort', 1);
            } else {
                ServicesModel::whereBetween('sort', [$from, $to])->whereNotIn("id", [$get->id])->decrement('sort', 1);
            }
            $query = ServicesModel::where('id', $get->id)->update(['sort' => $to]);
