...
class TrucktypeController extends Controller
{
    ...
    public function dragsort(Request $request)
    {
        ...
        $get = TrucktypeModel::find($request->id);
        if ($from != "" && $to != "") {
            if ($from > $to) {
                TrucktypeModel::whereBetween('sort', [$to, $from])->whereNotIn("id", [$get->id])->increment('sort', 1);
            } else {
                TrucktypeModel::whereBetween('sort', [$from, $to])->whereNotIn("id", [$get->id])->decrement('sort', 1);
            }
            $query = TrucktypeModel::where('id', $get->id)->update(['sort' => $to]);
