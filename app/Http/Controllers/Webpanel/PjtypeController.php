...
class PjtypeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($file) {
            ...
            $newLg = 'upload/pjtype/' . $filename . '-.' . $ext;
        ...
        if ($data->save()) {
            ...
            if ($request->gallery) {
                ...
                for ($i = 0; $i < count($gallery); $i++) {
                    ...
                    $newLg = 'upload/pjtype/gallery/' . $gfilename . '-' . $i . '.' . $ext;

                    Storage::disk('public')->put($newLg, $lg);

                    GalleryModel::insert(['_id' => $data->id, 'type' => 'pjtype', 'image' => $newLg, 'created' => date('Y-m-d H:i:s')]);
                }
            }
