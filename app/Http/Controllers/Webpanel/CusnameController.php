...
class CusnameController extends Controller
{
    ...
    protected $controller = 'cusname';
    protected $folder = 'cusname';
    ...
    public function index(Request $request)
    {
        ...
        return view("$this->prefix.pages.cusname.index",[
            'css'=> ['back-end/css/table-responsive.css'],        
            'js' => [
                ['type'=>"text/javascript",'src'=>"back-end/js/jquery.min.js",'class'=>"view-script"],
                ["src"=>"back-end/js/table-dragger.min.js"],
                ["src"=>'back-end/js/sweetalert2.all.min.js'],
                ["type"=>"text/javascript","src"=>"back-end/build/cusname.js"],
            ],
            'prefix' => $this->prefix,
            'folder' => 'cusname',
            'page' => 'index',
            'segment' => "$this->segment/cusname",
            'rows' => $rows
        ]);
    }
    ...
    {
        $datas = CusModel::find(explode(',', $request->id));
        if (@$datas) {
            foreach ($datas as $data) {

                CusModel::where('sort', '>', $data->sort)->decrement('sort');
                //destroy
                $query = CusModel::destroy($data->id);
            }
        }
        if (@$query) {
    ...
    public function dragsort(Request $request)
    {
        $from = $request->from;
        $to = $request->to;
        $data = CusModel::find($request->id);

        if($from!="" && $to !="")
        {
            if($from > $to){
                CusModel::whereBetween('sort', [$to, $from])->whereNotIn('id',[$data->id])->increment('sort');
            }else{
                CusModel::whereBetween('sort', [$from, $to])->whereNotIn('id',[$data->id])->decrement('sort');
            }
            $data->sort = $to;
            if($data->save()){
                return response()->json(true);
